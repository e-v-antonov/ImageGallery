...
namespace ImageGallery.AdditionalClass
{
    ...
    class Filter
    {
        public static UInt32[,] matrix_filtration(int W, int H, UInt32[,] pixel, int N, double[,] matryx)
        {
            ...
            UInt32[,] tmppixel = new UInt32[tmpW, tmpH];
            UInt32[,] newpixel = new UInt32[W, H];
            ...
            for (i = 0; i < gap; i++)
            {
                for (j = 0; j < gap; j++)
                {
                    ...
                    tmppixel[i, tmpH - 1 - j] = pixel[0, H - 1];
                    tmppixel[tmpW - 1 - i, j] = pixel[W - 1, 0];
                    tmppixel[tmpW - 1 - i, tmpH - 1 - j] = pixel[W - 1, H - 1];
                }
            }
                
            //крайние левая и правая стороны
            for (i = gap; i < tmpW - gap; i++)
            {
                for (j = 0; j < gap; j++)
                {
                    tmppixel[i, j] = pixel[i - gap, j];
                    tmppixel[i, tmpH - 1 - j] = pixel[i - gap, H - 1 - j];
                }
            }
            ...
            for (i = 0; i < gap; i++)
            {
                for (j = gap; j < tmpH - gap; j++)
                {
                    tmppixel[i, j] = pixel[i, j - gap];
                    tmppixel[tmpW - 1 - i, j] = pixel[W - 1 - i, j - gap];
                }
            }
